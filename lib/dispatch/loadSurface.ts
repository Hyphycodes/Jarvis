...
export const loadTodaySurface: Loader<TodayPayload> = async () => {
  ...
  return {
    hero: {
      ...
      date: formatToday(),
      greeting: planRow
        ? `Tonight: ${planRow.title}.`
        : "Quiet day.",
      summary:
        planRow?.summary ??
        "Nothing strong enough to surface yet.",
      primaryPlanId: planRow?.id,
      leaveBy:
        isRecord(planRow?.key_stats) && typeof planRow.key_stats.leave_by === "string"
...
export const loadNorthSurface: Loader<NorthPayload> = async () => {
  const { id } = await getViewableProfileId();
  if (!id) return { northStar: { title: "North", subtitle: "" }, pillars: [], signals: [] };

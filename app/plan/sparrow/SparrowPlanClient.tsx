...
export type SparrowPlanClientProps = {
  planId: string | null;
  initialLive: boolean;
  dateLabel: string;
};

export function SparrowPlanClient({
  planId,
  initialLive,
  dateLabel,
}: SparrowPlanClientProps) {
  const router = useRouter();
  const [pending, startTransition] = useTransition();

  const ctaLabel = initialLive ? "Live" : "Begin Evening";
  ...
  function onBegin() {
    ...
    startTransition(async () => {
      try {
        await fetch(`/api/plans/${planId}/live`, {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({ enabled: true }),
        });
      } catch (err) {
        console.error("begin evening failed", err);
      }
      router.push("/active/sparrow");
    });
  }
    ...
    <div className="relative -mx-6 -mt-[calc(env(safe-area-inset-top)+24px)] overflow-hidden">
      ...
      >
        ...
        <button
          ...
          disabled={pending}
          className={
            "mb-8 mt-2 flex items-center justify-between rounded-[4px] px-6 py-4 text-[12px] uppercase tracking-editorial transition duration-300 ease-atmospheric active:translate-y-px disabled:opacity-70 " +
            (live

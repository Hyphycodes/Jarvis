...
function DropItIn() {
  ...
  async function handleSubmit() {
    if (!text.trim() && !url.trim() && !imageFile) return;
    setLoading(true);
    setResult(null);
    ...
    try {
      ...
      if (image_base64) {
        ...
        body.image_media_type = image_media_type ?? "image/jpeg";
      }

      const res = await fetch("/api/intake/drop", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(body),
      });

      const data = (await res.json()) as DropResult;
      setResult(data);
    } catch (err) {
      setResult({
        ok: false,
  ...
  return (
    <section className="lux-surface mt-10 rounded-[var(--radius-card)]">
      {/* Header row */}
      <button
        type="button"
        onClick={() => setOpen((s) => !s)}
        ...
        aria-expanded={open}
      >
        <span className="text-[11px] uppercase tracking-[0.22em] text-warm-ivory/72">
          Drop it in
        </span>
        <span className="text-warm-ivory/55">
          {open ? (
            ...
            <span className="text-[18px] leading-none text-muted-gold/70">+</span>
          )}
        </span>
      </button>
      ...
      {open ? (
        <div className="flex flex-col gap-4 px-5 pb-5">
          ...
          <textarea
            ...
            placeholder="Paste a link, describe what you heard, or drop flyer text…"
            rows={3}
            className="w-full resize-none rounded-[var(--radius-soft)] border border-white/[0.07] bg-white/[0.03] px-4 py-3 text-[14px] leading-[1.55] text-warm-ivory/88 placeholder:text-warm-ivory/30 focus:outline-none focus:ring-1 focus:ring-muted-gold/40"
          />
          ...
          <input
            ...
            className="w-full rounded-[var(--radius-soft)] border border-white/[0.07] bg-white/[0.03] px-4 py-2.5 text-[13px] text-warm-ivory/88 placeholder:text-warm-ivory/30 focus:outline-none focus:ring-1 focus:ring-muted-gold/40"
          />
          ...
          <div className="flex items-center gap-3">
            ...
            <label
              ...
              className="cursor-pointer rounded-[var(--radius-soft)] border border-white/[0.07] px-3 py-2 text-[11px] uppercase tracking-[0.16em] text-warm-ivory/55 transition-colors hover:text-warm-ivory/80"
            >
              {imageFile ? imageFile.name.slice(0, 24) : "Add screenshot"}
            </label>
            {imageFile ? (
              <button
                type="button"
          ...
          <button
            ...
            disabled={loading || (!text.trim() && !url.trim() && !imageFile)}
            className="lux-action w-full rounded-full py-3 text-[11px] uppercase tracking-[0.18em] disabled:opacity-40"
          >
            {loading ? "Researching…" : "Drop it in"}
          </button>

          {/* Result */}
          {result ? (
            <div className="flex flex-col gap-3 rounded-[var(--radius-soft)] border border-white/[0.07] bg-white/[0.02] px-4 py-4">
              {result.ok && result.verdict ? (
                <>
                  {result.venue_name ? (
                    <div className="text-[11px] uppercase tracking-[0.2em] text-muted-gold/70">
                      {result.venue_name}
                    </div>
                  ) : null}
                  <p className="text-[14px] leading-[1.55] text-warm-ivory/88">
                    {result.verdict.verdict}
                  </p>
                  <div className="flex items-center justify-between">
                    <span className="inline-flex items-center rounded-sm border border-muted-gold/30 px-2.5 py-1 text-[10px] uppercase tracking-[0.18em] text-muted-gold/70">
                      {result.action_recommendation}
                    </span>
                    <button
                      type="button"
                      onClick={handleClear}
                      className="text-[11px] text-warm-ivory/35 hover:text-warm-ivory/60"
                    >
                      Clear
                    </button>
                  </div>
                ...
                <>
                  <p className="text-[13px] text-warm-ivory/55">
                    {result.error ?? "Couldn't find enough signal on this one."}
                  </p>
                  <button
                    type="button"
                    onClick={handleClear}
                    className="self-start text-[11px] text-warm-ivory/35 hover:text-warm-ivory/60"
                  >
                    Clear
                  </button>
                </>
